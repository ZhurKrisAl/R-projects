...
Выгружаем данные и удаляем признак `Density`, так как зависимая переменная `BodyFat` входит
в формулу определения плотности тела и получается, что результаты будут
ненадёжными, если не уберём её из исследования. Очищаем их от пропущенных значений и предварительно просматриваем их:
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
Согласно результатам, большинство переменные незначимы для регрессии (за исключением запястья, предплеья, возраста, живота и шеи), но это противоречит логике, поэтому мы будем пренебрегать этим. А значение скорректированного коэффициента детерминации равно 0.726, что является неплохим результатом.
...
В нашу регрессию буду входить запястье, возраст, вес, живота, все части бедра, предплечье и бицепсов.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
lm2 <- lm(BodyFat ~ Wrist+Age+Weight+Abdomen+Hip+Biceps+Thigh+Forearm, df5)
pander(summary(lm2))
```

Скорректированный коэффицент детерминации чуть-чуть, но уменьшилось, при этом количество параметров тоже уменьшилось.

Также можем построить регрессию с запястьем, предплеьем, возрастом, шея и живот, так как это единственные параметры, которые входе теста оказались **значимым**.

```{r, echo=FALSE, message=FALSE, warning=FALSE}
lm3 <- lm(BodyFat ~ Forearm + Age + Wrist + Neck + Abdomen, df5)
pander(summary(lm3))
```

Этот модель по скорректированному коэффициенту детерминации является худщим из всех вышеперечисленных, так как оно упало аж на 0.01.
...
Наименьшее AIC и BIC имеет модель **lm2**, где признаки были отобраны по **методу включения**. В этой модели скорректированный коэффициент детерминации чуть хуже **lm1**, но при этом имеет гораздо меньше признаков, что способствует эффективности модели. Еще с помощью данного метода мы уменьшаем влияние мультиколинеарности (можно сказать в этой модели она отсутствует).
...
*P_value* больше 0.05, следовательно гипотеза о подчинении остатков к нормальному закону распределения **не отвергается**.
...
$$ y_{BodyFat} = -31.56 - 1.813 \cdot x_{Wrist} + 0.06415 \cdot x_{Age} - 0.1195 \cdot x_{Weight} + 0.2786 \cdot x_{Thigh} + 0.9101 \cdot x_{Abdomen} - 0.1434 \cdot x_{Hip} + 0.406 \cdot x_{Forearm} + 0.1283 \cdot x_{Biceps}$$

Уравнение регрессии имеет $\beta_{0}$ равной -31.56, которые по модулю имеет самое большое значение.

Посчитаем коэфициенты эластичности

```{r, echo=FALSE, message=FALSE, warning=FALSE}
paste("Эластичность Wrist:", round(( - 1.813*mean(df5$Wrist)/mean(df5$BodyFat)),3),"%")
paste("Эластичность Age:", round(( 0.06415*mean(df5$Age)/mean(df5$BodyFat)),3),"%")
paste("Эластичность Weight:", round((-0.1195*mean(df5$Weight)/mean(df5$BodyFat)),3),"%")
paste("Эластичность Thigh:", round((0.2786*mean(df5$Thigh)/mean(df5$BodyFat)),3),"%")
paste("Эластичность Abdomen:", round((0.9101*mean(df5$Abdomen)/mean(df5$BodyFat)),3),"%")
paste("Эластичность Hip:", round((-0.1434*mean(df5$Hip)/mean(df5$BodyFat)),3),"%")
paste("Эластичность Forearm:", round((0.406*mean(df5$Forearm)/mean(df5$BodyFat)),3),"%")
paste("Эластичность Biceps:", round((0.1283*mean(df5$Biceps)/mean(df5$BodyFat)),3),"%")
```

Наибольший по модулю коэффициент эластичности имеет показатель $Abdomen$, а значит, что при фиксированных значениях остальных переменных, уровень жира в теле увеличится на 4,419%, если плотность тела увеличится на 1%.

Наименьший по модулю коэффициент эластичности имеет показатель $Age$, а это значит, что при фиксированных значениях остальных переменных,уровень жира в теле увеличится всего на 0,151%, если возраст увеличится на 1%.
...
1.1) Рассмотрим степенную модель множественной регрессии со всеми имеющимися объясняющими переменными: $y = \beta_{0}\prod_{j=1}^{m}x_{j}^{\beta_{j}} \varepsilon$
...
1.2) Проверим остатки на нормальность распредления:

```{r}
pander(shapiro.test(lm1_1$residuals))
pander(jarque.bera.test(lm1_1$residuals))
```
Во всех случаях тесты на нормальность распределения остатков отвергаются на уровне значимости 0,05, т.к. $p-value < 0,05$.

1.3) Проверим остатки на гомоскедастичность:
```{r}
pander(bptest(lm1_1))
```
$p-value<0,05$, значит, гомоскедастичность отсутствует.

*2.1) Рассмотрим модель только со значимыми коэффициентами, то есть с теми, которые имеют **p-value \< 0.05**:*

```{r, echo=FALSE, message=FALSE, warning=FALSE}
lm2_1 <- lm(BodyFat ~ Neck + Abdomen + Hip+Wrist, df6_2)
pander(summary(lm2_1))
```

**Вывод:** скорректированный коэффициент детерминации (Adjusted R2) уменьшилcя, значит, регрессионная модель ухудшилась.

2.2) Проверим остатки на нормальность распредления:

```{r}
pander(shapiro.test(lm2_1$residuals))
pander(jarque.bera.test(lm2_1$residuals))
```
Во всех случаях тесты на нормальность распределения остатков отвергаются на уровне значимости 0,05, т.к. $p-value < 0,05$.

2.3) Проверим остатки на гомоскедастичность:
```{r}
pander(bptest(lm2_1))
```
$p-value<0,05$, значит, гомоскедастичность отсутствует.
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
lm3_1 <- lm(BodyFat ~  Age + Weight + Neck + Abdomen + Hip + Thigh + Forearm + Wrist , df6_2)
...
3.2) Проверим остатки на нормальность распредления:

```{r}
pander(shapiro.test(lm3_1$residuals))
pander(jarque.bera.test(lm3_1$residuals))
```
Во всех случаях тесты на нормальность распределения остатков отвергаются на уровне значимости 0,05, т.к. $p-value < 0,05$.

3.3) Проверим остатки на гомоскедастичность:
```{r}
pander(bptest(lm3_1))
```
$p-value<0,05$, значит, гомоскедастичность отсутствует.
